...
import { ChevronLeft, ChevronRight, Heart, MapPin, Shield, UserRound, LogOut } from 'lucide-react';
...
export default function Profile() {
  ...
  return (
    <div className="mx-auto w-full max-w-lg min-h-full overflow-y-auto bg-bg text-text-primary">
      {/* Redesigned Header Banner */}
      <div className="bg-gradient-to-r from-accent to-violet-600 px-6 pt-6 pb-8 rounded-b-[2.5rem] shadow-card relative">
        {/* Top bar controls */}
      ...
      <div className="px-4 py-6 space-y-6 pb-24">
        {message && (
          <p className="rounded-2xl border border-danger/25 bg-danger/5 px-4 py-3 text-sm text-danger">{message}</p>
        )}
        ...
        {showPromo && (
          <motion.div 
            ...
            className="surface-card rounded-[22px] p-5 border border-border-default bg-surface shadow-card relative overflow-hidden"
          >
...
function Settings({ size }) {
  return (
    <svg 
      className="lucide lucide-settings animate-spin-hover" 
      width={size} 
      height={size} 
      viewBox="0 0 24 24" 
      fill="none" 
...
function ProfileSkeleton() {
  return (
    <div className="w-full max-w-lg mx-auto bg-bg animate-pulse">
      <div className="h-48 rounded-b-[2.5rem] bg-accent/25 relative flex flex-col items-center justify-end pb-8">
        <div className="h-24 w-24 rounded-full bg-white/20 skeleton" />
        <div className="h-5 w-36 bg-white/20 rounded-full mt-3 skeleton" />
        <div className="h-3 w-28 bg-white/10 rounded-full mt-1.5 skeleton" />
      </div>
