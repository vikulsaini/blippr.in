...
export async function sendPasswordResetEmail(email, code) {
  if (!resendReady()) {
    console.warn('Email provider is not configured. Set RESEND_API_KEY and EMAIL_FROM to send password reset emails.');
    return { sent: false, reason: 'email_not_configured' };
  }
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: process.env.EMAIL_FROM,
      to: email,
      subject: 'Reset your Blippr password',
      text: `Your Blippr password reset code is ${code}. It expires in 10 minutes.`
    })
  });
